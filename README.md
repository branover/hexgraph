...
| **Fuzzing** | `hexgraph config set features.fuzzing.enabled true` (needs `just fuzz-build`) | **Coverage-guided, first-class (AFL++ + libFuzzer), campaign-driven.** The `fuzzing` *task* is a single libFuzzer+ASan pass; a **campaign** is a long-lived, detached job. The `Fuzzer` seam picks the engine by attack surface: a Phase-2 **instrumented derived target** (with source) → **AFL++** (`afl-clang-lto` + CmpLog magic-byte gating + persistent mode) on the code under test = real coverage feedback (vs. coverage-blind black-box on a plain `.so`). **Campaigns are detached + crash-safe**: each launches a hardened `docker run -d` container owned by a durable `fuzz_campaign` row; a periodic reaper streams crashes → `fuzz_crash` findings as they happen (a 6-hour run surfaces its first crash in minutes), updates live stats/coverage, dedups by a normalized stack-hash, minimizes the reproducer (`afl-tmin`/libFuzzer), classifies exploitability, and **survives a `serve` restart** (the reaper re-attaches by container name). **Start/stop/resume** preserves the corpus in CAS (AFL++ resumes natively). A crash's minimized reproducer is **re-verifiable in one click** via the existing `verify_poc` path (LLM-free). Resource ceilings are a user-tunable **`ResourceSpec`** (`{mem,cpus,pids,tmpfs,timeout,unconstrained}`, a Settings default + per-campaign override); `unconstrained` lifts mem/cpu/pids ONLY to use the whole machine — **never a security relaxation** (`--network none`, cap-drop, no-new-privileges, read-only, non-root all still hold). Runs in the dedicated `hexgraph-fuzz` image (`just fuzz-build`). All new structure rides `evidence.extra.fuzz` + the `fuzz_campaign`/`fuzz_artifact` tables. **Relaxes the static-only policy to allow execution** (still `--network none`, capped, timed). |
...
| `HEXGRAPH_FUZZ_IMAGE` | `hexgraph-fuzz:latest` | Coverage-guided fuzz image (`features.fuzzing` campaigns; AFL++ + libFuzzer + llvm-symbolizer). Point at a private tag in a worktree — never clobber the shared tag. |
| `HEXGRAPH_FUZZER` | _(by surface)_ | Force the Fuzzer seam to the offline `mock` engine (tests/$0); otherwise the seam picks AFL++/libFuzzer by attack surface. |
