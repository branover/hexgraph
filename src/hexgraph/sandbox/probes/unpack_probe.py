...
# no signal — flagging it lets the ingest summary SAY "promote this to go deeper" (a Cisco
# IOS-XE image, e.g., leaves the iosd/WebUI runtime in nested .pkg/squashfs).
_CONTAINER_MAGICS = (
...
def _container_format(head: bytes, name: str) -> str | None:
    ...
    for a Cisco IOS-XE `.pkg`/`.SPA.pkg` (squashfs/cpio under a signed wrapper the magic misses)."""
