...
class BuildSpec:
    ...
    network: str = "none"                           # "none" (default, only valid value this phase)
    timeout: int = DEFAULT_BUILD_TIMEOUT
    name: str = "build"

    def recipe_sha(self) -> str:
        """sha256 over {phases, env, base_image, instrumentation, arch} — the recipe
        identity. Deterministic: sorted keys, canonical JSON, so the SAME recipe
        always hashes the same regardless of dict/tuple ordering."""
        basis = {
...
class Builder(Protocol):
    ...
    def build(self, spec: BuildSpec, *, source_root: str, content_hash: str | None = None) -> BuildResult:
        ...
...
def instrumentation_env(instr: Instrumentation) -> dict[str, str]:
    """The injected toolchain env for an instrumentation profile (the OSS-Fuzz
    lesson: separate 'what to build' from 'how it's instrumented'). Vendored/offline
    — no fetch. Returns the env the probe exports before running the recipe phases."""
...
class SandboxBuilder(Builder):
    ...
    def build(self, spec: BuildSpec, *, source_root: str, content_hash: str | None = None) -> BuildResult:
        ...
        inj_env = instrumentation_env(spec.instrumentation)
        merged_env = {**inj_env, **dict(spec.env)}  # recipe env can override (rare)
        probe_payload = {
        ...
        try:
            result = executor.run_probe(
                ...
                # The build runs a compiler in /scratch (exec tmpfs) but does NOT execute
                # the TARGET — the build gate (asserted above) is independent of the exec
                # gate, so we pass requires_execution=False (network stays none).
        ...
        return self._finalize(spec, recipe_sha, content_hash, data, outdir)
