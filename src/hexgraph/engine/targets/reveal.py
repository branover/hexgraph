...
def set_visible(session: Session, project_id: str, target_id: str, visible: bool, *,
    ...
    # on `not was_visible` meant a target that was already revealed could never be enriched —
    # only an undocumented hide-then-reveal dance worked. `enrich=True` is already an explicit
    # per-call opt-in on ONE named target, so honoring it on an already-visible target is what the
    # caller asked for. (reveal_dir deliberately does NOT reconsider visible children — with
    # prefix="" that's the dozen-analyses stampede its own docstring records; it points here.)
    wants_enrich = bool(visible and enrich)
    if visible and (not was_visible or wants_enrich):
        project = session.get(Project, project_id)
        enrichment_queued = _materialize_on_reveal(session, project, t, enrich=wants_enrich)
        materialized = True
    session.flush()
    return {"target_id": t.id, "name": t.name, "visible": t.visible,
            "materialized": materialized, "enrichment_queued": enrichment_queued}
