...
_CATALOG = [
    ...
    ("read", "list_source_trees", _t.list_source_trees, "List the project's managed SOURCE trees (trusted source we possess/build — NOT the hostile target). Harnesses, PoCs, and build/run scripts now live here as role-tagged source_files. Returns id/name/origin/file_count + which targets are built_from each. Use read_source_file to view a tree's files.",
     {"type": "object", "properties": {"project_id": {"type": "string"}}, "required": ["project_id"]}),
    ("read", "read_source_file", _t.read_source_file, "Browse/read a managed SOURCE tree (tree_id from list_source_trees). Omit `rel` to LIST files (rel/size/role); pass `rel` to READ one file's text (bounded, traversal-safe; binary as hex). Trusted source text (a harness/PoC/build recipe / imported library source) — distinct from read_file, which reads a firmware's hostile unpacked bytes.",
     {"type": "object", "properties": {"tree_id": {"type": "string"}, "rel": {"type": "string"}}, "required": ["tree_id"]}),
    ("read", "list_builds", _t.list_builds, "List builds in a project — the build ledger. Each carries status, the reproducibility triple (recipe_sha/source_content_hash/toolchain_digest), artifacts as CAS shas, and the instrumented derived_target_id it registered. Optionally filter by source_tree_id. Returns {build_specs, builds}.",
    ...
    ("read", "get_schemas", _t.get_schemas, "The write-API contract: allowed enums, the Finding shape, per-type NODE attribute schemas (what to populate, the sink-vs-symbol rule), edge/socket attribute schemas, and the active decompiler. Read before record_finding/create_node/create_edge/annotate to avoid guessing.",
     {"type": "object", "properties": {}}),
    ("write", "import_source_tree", _t.import_source_tree, "Create a managed SOURCE tree and optionally populate it. `files` = [{rel,content,role?}] (role in code|harness|poc|script|build_recipe). Use to bring in a harness/PoC you authored or a small library's source. Trusted text only — NOT target bytes (those are ingested as targets). Returns {id,name,written}.",
     {"type": "object", "properties": {"project_id": {"type": "string"}, "name": {"type": "string"}, "files": {"type": "array"}, "origin": {"type": "string"}}, "required": ["project_id", "name"]}),
    ("write", "link_finding_to_source", _t.link_finding_to_source, "Link a finding to its location in a managed source file (a `located_in` edge + evidence.extra.source_ref) — the jump-from-finding-to-source link. tree_id/rel from list_source_trees/read_source_file; pass `line` to scroll there. Use when a vuln/harness corresponds to known source.",
     {"type": "object", "properties": {"finding_id": {"type": "string"}, "tree_id": {"type": "string"}, "rel": {"type": "string"}, "line": {"type": "integer"}, "col": {"type": "integer"}}, "required": ["finding_id", "tree_id", "rel"]}),
    ...
    ("run", "build_target", _t.build_target, "Build a managed SOURCE tree into an INSTRUMENTED artifact in the sandbox via a RECORDED, REPRODUCIBLE recipe (build-as-API): you author/approve a BuildSpec and REQUEST the build — you never run a compiler, HexGraph runs the recipe. `system` (make|cmake|autotools|meson|cargo|go|custom, auto-detected if omitted), `phases` (ordered explicit-argv steps, recorded verbatim; defaults derived from the system), `instrumentation` ({sanitizers:[address,undefined,…], coverage:[sancov|afl_pcguard], engine:libfuzzer|afl}), `artifacts` (rel paths to capture), `env` (NON-secret — secrets rejected). Instrumentation is INJECTED as CC/CXX/CFLAGS by HexGraph (base-image contract), so the SAME phases yield ASan/SanCov/AFL++ builds by swapping the profile. recipe_sha=hash{phases,env,base_image,instrumentation,arch}; same recipe_sha+source content_hash+toolchain_digest ⇒ same build. If the tree is built_from a target, the rebuild is registered as a DERIVED target wired instrumented_build_of→ the original (ready for coverage-guided fuzzing next). VENDORED/OFFLINE ONLY (--network none; a net-dep recipe fails honestly). Requires features.build.",
     {"type": "object", "properties": {"project_id": {"type": "string"}, "source_tree_id": {"type": "string"}, "system": {"type": "string"}, "phases": {"type": "array"}, "instrumentation": {"type": "object"}, "artifacts": {"type": "array"}, "env": {"type": "object"}, "arch": {"type": "string"}}, "required": ["project_id", "source_tree_id"]}),
