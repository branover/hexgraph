...
> syntax highlighting) — both deferred per `docs/dev/ui-backlog.md`.

**Phase 2 — Builder seam + build-as-API (gated `features.build`).** `engine/build.py` + `SandboxBuilder` + `build_detect_probe.py`/`build_probe.py` + `docker/build.Dockerfile`/`hexgraph-build`; `assert_allows_build()` (the only `policy.py` edit); `build_spec`/`build` tables (0013); `BuildSpec` model + CAS storage + the base-image contract; rebuild-VR-target-with-instrumentation → derived target; `/api/builds` + `build_target` MCP run-tool; the IDE Build modal (recorded-recipe preview, instrumentation toggles, vendored-only). **Vendored/offline only.** *Risk:* supply-chain — contained by sandbox `--network none` compile, non-root RO source, ephemeral container.
...
> image (`docker/build.Dockerfile` / `just build-image` [D4] — clang/LLVM + sanitizer/SanCov runtimes +
...
> (`docker/fuzz.Dockerfile`/`just fuzz-build`; worktree builds a PRIVATE tag, `HEXGRAPH_FUZZ_IMAGE`).
...
> the Settings `resources` section — a shared `resources.default` ← per-container-type
> `resources.{sandbox,build,fuzzing}` override, + a per-campaign override for fuzzing): `unconstrained` drops
...
> ResourceSpec, the Build modal, Settings, deep-links) — screenshots judged in `docs/dev/ui-backlog.md`.
...
> `docker/fuzz.Dockerfile` builds AFL++ from source with qemu-mode (afl-qemu-trace) + frida-mode, preeny/
...
> `docker/build.Dockerfile` under `WITH_CROSS=1`. **(3) Determinism + cache:** `SOURCE_DATE_EPOCH`
...
> the Build modal's arch/dependency posture (`docs/dev/ui-backlog.md`). **Known limits:** ccache/
