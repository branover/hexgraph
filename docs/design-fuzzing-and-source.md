...
- **Gate creep:** every capability maps to an existing tier; the **only** gate edits are `allow_build`/`allow_build_fetch` + their asserts in `policy.py`. No feature code branches on tier, backend, engine, or executor identity.
