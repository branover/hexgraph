...
def test_container_format_detects_nested_containers():
    ...
    # large vendor firmware packages: squashfs/cpio under a signed wrapper the magic misses → flag by ext.
    assert up._container_format(b"\x00" * 8, "vendor-fw-1.0.SPA.pkg") == "vendor-pkg"
    # Plain ELF / ordinary file → not flagged.
    assert up._container_format(b"\x7fELF\x02\x01\x01\x00", "svcd") is None
...
def test_packed_containers_helper_sorts_and_filters():
    files = [
        ...
        {"rel": "usr/sbin/svcd", "is_elf": True, "size": 5000},                 # not a container
        {"rel": "blob.squashfs", "container": "squashfs", "size": 999999},
        {"rel": "pkgA.SPA.pkg", "container": "vendor-pkg", "size": 50000},
    ...
    assert [c["rel"] for c in out] == ["blob.squashfs", "pkgA.SPA.pkg", "boot/initramfs"]
...
def test_record_manifest_persists_container_tag():
    ...
    record_manifest(fw, method="binwalk", root_rel="", files=[
        {"rel": "blob.squashfs", "container": "squashfs", "size": 999999},
        {"rel": "usr/sbin/svcd", "is_elf": True, "size": 5000},  # ordinary file → no container key
    ])
    ...
    assert [c["rel"] for c in packed_containers(persisted)] == ["blob.squashfs"]
