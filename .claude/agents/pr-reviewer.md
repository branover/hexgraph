...
## 2. Run BOTH skills
Invoke the Skill tool with skill `code-review` (args `high`) and skill `security-review`. They are available because this agent declares them.
...
b. **Add line-level inline comments** for the concrete findings (best-effort bonus, on top of the summary): `gh api repos/<owner>/<repo>/pulls/<N>/comments` with `commit_id` (the PR head sha), `path`, `line`, `side=RIGHT`, `body`. If an inline post errors, skip it and move on — the summary already carries the finding.
