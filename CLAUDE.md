...
- **`LLMBackend`** (`llm/`, selected by `HEXGRAPH_LLM_BACKEND`, default `mock`): `MockLLMBackend` / `AnthropicAPIBackend` / `ClaudeCodeBackend`. **Never write `if backend == "mock"` in task code.**
