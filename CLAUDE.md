...
- **Never commit or push to `main`.** `main` only changes by **merging a reviewed PR**. This is now enforced by the **`protect-main` repository ruleset** (not just discipline): `main` requires a pull request with **1 approving review**, **all review threads resolved**, the **required status checks green** (`tests (offline, py3.11)`, `tests (offline, py3.12)`, `frontend build` — strict, so the branch must be up to date with `main`), and **linear history** (no merge commits; direct pushes, force-pushes, and deletion of `main` are blocked). Stale approvals are dismissed on a new push.
...
4. Only after the review passes (its PR comments addressed **and the review threads resolved**) and **CI is green**: merge with **`gh pr merge --squash --delete-branch`** (or `--rebase`; **not `--merge`** — the ruleset requires linear history, and `--delete-branch` removes the branch **both locally and on the remote**). Because the required status checks are **strict**, rebase the branch onto the latest `main` (and let CI re-run) before merging if `main` has moved. CI (`.github/workflows`) now runs the offline test matrix + frontend build + a dependency audit + the live sandbox tests on every PR; the review **plus** green CI is the gate. (Single-owner caveat: GitHub won't let the repo owner approve their own PR, so the 1-approval requirement can only be met by a second reviewer's approval or an owner **`gh pr merge --admin`** bypass — use the bypass only with the maintainer's go-ahead, never to skip an unfinished review.)
