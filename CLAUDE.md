...
- **Never commit or push to `main`.** `main` only changes by **merging a reviewed PR**. This is now enforced by the **`protect-main` repository ruleset** (not just discipline): `main` requires a pull request, **all review threads resolved**, the **required status checks green** (`tests (offline, py3.11)`, `tests (offline, py3.12)`, `frontend build` — strict, so the branch must be up to date with `main`), and **linear history** (no merge commits; direct pushes, force-pushes, and deletion of `main` are blocked). The required **approving-review count is 0** (set deliberately 2026-06-05: this is a single-owner repo and GitHub won't let the owner approve their own PR, so a non-zero count could only be met by an `--admin` bypass that *also* skips the strict CI gate — exactly what we avoid). **Do not use `gh pr merge --admin`** — there is nothing for it to satisfy, and its only effect would be to bypass the still-strict CI checks. Merge access is gated by collaborator **write** permission (owner-only today), so 0 approvals does **not** let a random/fork PR self-merge; the **independent-review merge gate below is still required by discipline** (it is simply no longer GitHub-enforced via the approval count). If write collaborators are ever added, reconsider raising the count back to 1 (or adding `CODEOWNERS`).
...
2. **A review subagent — a *different* agent than whoever wrote the code — must review the PR diff** (correctness, the security invariants [loopback / sandbox / secret-never-logged / the opt-in execution policy], test quality, and that docs/migrations were updated). **Who launches it depends on who can:**
   ...
   - **Spawn the reviewer with the review skills wired in — otherwise it can't run them.** A subagent can only invoke a skill that its **agent definition declares**; a generic agent spawned with no skills will find `/code-review` and `/security-review` unavailable and silently fall back to an ad-hoc manual review (weaker, and the gate expects both skills run). So a PR-review subagent must be given the `Skill` tool **and** the review skills in its frontmatter — e.g. an agent defined under `.claude/agents/`:
     ```
     ---
     name: pr-reviewer
     description: Independent PR reviewer — correctness, security invariants, test/doc/migration completeness
     ...
     ---
     ```
     then dispatch it via the `Agent` tool's `subagent_type` (here `pr-reviewer`). Tools and skills are illustrative — **pick the set that fits the subagent's job**: a reviewer needs `code-review` + `security-review`; a different subagent gets whatever skills its task actually calls for (and only those, to keep its context lean). The reviewer must still **invoke both `/code-review` and `/security-review`** on the PR diff and post their output per step 3.
...
4. Only after the review passes (its PR comments addressed **and the review threads resolved**) and **CI is green**: merge with **`gh pr merge --squash --delete-branch`** (or `--rebase`; **not `--merge`** — the ruleset requires linear history, and `--delete-branch` removes the branch **both locally and on the remote**). Because the required status checks are **strict**, rebase the branch onto the latest `main` (and let CI re-run) before merging if `main` has moved. CI (`.github/workflows`) now runs the offline test matrix + frontend build + a dependency audit + the live sandbox tests on every PR; the review **plus** green CI is the gate. **No `--admin`, ever** — with 0 required approvals a plain `gh pr merge --squash --delete-branch` works the moment CI is green, and GitHub refuses it until then, so a plain merge can never bypass CI (an `--admin` merge could, which is why it's off the table). If you reach for `--admin`, stop: either CI isn't green yet (wait) or a thread is unresolved (resolve it).
...
The MCP tool surface lives in `engine/mcp_catalog.py` (`_CATALOG` — the `(group, name, fn, description, json_schema)` tuples that are the agent-facing API; the thin implementations are in `mcp_tools.py`). The catalog **guard test, `tests/test_tool_contract.py`, enforces every rule below**, so a new tool that breaks one fails CI — when you add a tool, satisfy the guard rather than relaxing it.

- **Names are `<domain>_<verb>[_object]`, lowercase, routable from the name alone.** Under deferred (name-only) tool loading the *name* is the only zero-token signal an agent has, so it must telegraph the tool's purpose without a schema fetch. The domains: `proj` (projects) · `target` (the target lifecycle **and every tool that creates a target** — ingest/register/rehost) · `re` (static reverse engineering) · `fs` (a target's unpacked filesystem) · `obs` (the Observation store) · `graph` (the curated node/edge/hypothesis graph) · `finding` (findings, n-day, proving) · `src` (source trees + builds) · `fuzz` (campaigns) · `net` (live network interaction + egress) · `task` (the task runner) · `meta` (schemas + health). A new tool MUST take a known prefix; extend the domain set only if nothing genuinely fits (rare). Keep the verb distinction: **`ingest`** processes bytes into a target, **`register`** records a pre-existing live thing as a target. If two tools share a noun but produce different things, the type goes in the name (`graph_create_socket` — a graph node — vs `target_register_service` — a live, fuzzable target).
- **Two tool-name surfaces — never conflate them.** `mcp_catalog.py` holds the **advertised MCP names** (namespaced, external-agent-facing). `engine/agent_tools.py` holds the **in-process LLM agent-loop tools** (`decompile_function`, `list_observations`, …) that HexGraph's own LLM tasks call and the **mock fixtures reference by name** — those are deliberately **not** namespaced and must not be renamed to match the catalog. A catalog rename changes only the advertised `name` string; the tuple's 3rd element stays the real `mcp_tools` function (whose name never changes).
- **Closed value-sets are schema `enum`s generated from one source of truth — never hand-typed.** Build each enum by importing the canonical definition (`NodeType`/`EdgeType`, `SOCKET_KINDS`, `FINDING_TYPES`, the Finding `Literal`s, `hypotheses.RELATIONS`/`STATUSES`, …) at catalog module load — the **same** definitions `meta_get_schemas` reads — so the enum can't drift from the engine or the schema, and adding a vocab member updates both at once (the String columns keep zero-migration; the enum just mirrors today's vocab). **Hand-listing a value set in the catalog is the bug to avoid:** it silently drifts, and a strict MCP client then rejects a call the engine would have accepted (this exact drift — `relation`/`status`/`remote-tool` enums missing valid values — was caught only in review). **Code convention for anything new with a closed value set: define it as an importable constant** — a `str`-Enum, a module-level tuple/`frozenset`, or a `Literal` — in the engine/model that owns it, and import THAT into both `meta_get_schemas` and the catalog, so the schema enum is derived, not duplicated. A few legacy sets are still hand-listed because their authority isn't host-importable (e.g. `REMOTE_TOOLS` mirrors the sandbox-only `remote_probe.TOOLS`, which imports `_egress`); each carries a pointer comment **and** a guard-test assertion pinning it to that authority — match that pattern, but prefer a real importable constant for anything new.
- **Gating is stated in the description, in a fixed slot.** Any tool that touches the network, executes, boots an image, fetches deps, or edits source ends its description with **`Gated: features.X`** naming its `features.*` gate(s) — an agent must never discover a capability tier only by being refused (the gate is still enforced at the policy seam; the clause is so the agent can *plan*). A tool that should be **invisible until its feature is on** goes in `_FEATURE_GATED_TOOLS` (advertised conditionally), keeping the agent's context lean.
- **Descriptions lead with what the tool operates on, and disambiguate siblings.** The first clause names the corpus / operated-on type, especially for confusable families (the three searches — `graph_search`/`obs_search`/`re_search_decompiled`; the three file readers — `fs_read_file`/`src_read_file`/`net_remote_read_file`). Use the curation verbs consistently — **QUERY** (records an Observation, adds no graph nodes) / **PROMOTE** (deliberately adds a node) / **ENRICH** (attaches facts in place) — reference sibling tools by their current names, and keep chaining hints (`→ then …`) in prose. Put a one-line description on the non-obvious params (objects, arrays, spec-shaped ones); bare ids like `target_id` don't need one.
- **Keep the instructions in sync.** A tool rename or addition must propagate **in the same PR** to the VR **SKILL** (`agent_setup.py` — the standing instructions an external agent reads) and **`docs/mcp.md`**. The SKILL teaches the *strategic workflow* (orient cheaply → map the surface → read code → recover/deepen → prove), not just a tool list — slot a new tool into that flow, don't just append it. Design docs (`docs/design/`) reference tools as historical design artifacts and need not be renamed.
