...
2. Re-verify with `just test` (fast tier, mock, offline; `just test-heavy` for the slow real-container E2E tier) and `just demo` (full loop; needs Docker + sandbox image).
...
- **The repo is PUBLIC — NEVER commit real-engagement information.** Target identities and specifics of any real engagement — customer/vendor/product/model names, firmware filenames or versions, extracted artifact names (squashfs/package names, hashes), discovered attack-surface details, or a dogfood/field-test **report or plan about a real target** — stay LOCAL (e.g. under the engagement directory), never committed. In code, tests, docs, **commit messages, AND PR titles/descriptions/review comments**, use GENERIC or SYNTHETIC examples ("a large vendor firmware image", `vendor-pkg`, a made-up filename), never the real engagement's identifiers. The dogfood *skill* runs against real targets, but its findings + improvement report are LOCAL artifacts — only the resulting **generic** product changes get committed; the report/plan never goes in `docs/`. When in doubt, genericize. A public repo retains commit history **and merged-PR diffs forever**, so a leak can't be fully undone by a later history rewrite — keep it out in the first place.
...
**Coding-agent integration (MCP), two HexGraph directions that keep target bytes in the sandbox:**
...
- **Unrestricted skill** — `hexgraph-vr-unrestricted` is an additive sibling for native driver sessions only. It invokes the restricted primary workflow but may use separately authorized external tools when HexGraph is missing, gated, slow, or inferior, while retaining HexGraph as a best-effort ledger. It never enters delegate mode or bypasses a HexGraph gate; external operations do not inherit HexGraph's sandbox or Observation guarantees.
...
- `just test` (= `pytest -q -m "not slow"`, mock, offline — the FAST tier; deselects the heavy real-container E2E tests [`@pytest.mark.slow`: fuzz campaigns / rehost boots] that would otherwise run for minutes on an image-rich box; Docker-gated *unit* tests still skip without the image) · `just test-heavy` (the `slow` tier: real fuzz/build/rehost containers, with a per-test `--timeout` so a wedged container fails loudly instead of hanging the run) · `just demo` (full loop, needs Docker) · `just test-live` (real-key scored eval, needs `ANTHROPIC_API_KEY`, cassette-backed).
