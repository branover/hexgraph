...
- CLI (all working): `hexgraph init | db upgrade [--no-backup] | ingest <path> [--name] [--project] [--no-recon] | targets <p> | run <target> --type T [--objective] [--model] [--backend] [--function] [--mock-scenario] | findings <p> [--status] [--export f.json] | graph <p> --export f.json | serve`.
