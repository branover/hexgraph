...
**Workspace UX (graph + tasks + context):**
- Graph: **double-tap a node to collapse/expand its subtree** (firmware→binaries, binary→functions, node→findings; children derived from `contains`/`about` edges); **filter popover** (findings all/unresolved/none, toggle functions, expand-all). Function/symbol/string nodes tie to their binary via `target─contains→node` edges.
- **Deliberate LaunchModal** with a live pre-flight **context preview** (`POST /api/tasks/preview` → `context.preview_context`/`llm_tasks.preview_context`, no persist). Node-type-aware `NodeInspector`. Findings clicked in the graph scroll into view; a launched task auto-selects/scrolls. Tasks tab: timestamps, sort, type filter, **Clear** (`POST /api/projects/{id}/tasks/clear`). Right pane **expand-to-full-screen**. Finding-count badges on targets; finding lifecycle stepper.
- **Selective graph context** (`context._gather_graph_context`): a target's task bundle pulls THIS target's relationship edges (links_against/similar_to/duplicate_of/instance_of_pattern/related_to → other targets) + findings on those related targets (cross-target prior art) — bounded to direct neighbors, budget-packed, NOT the whole graph. (`ASSEMBLER_VERSION` = 2.)
